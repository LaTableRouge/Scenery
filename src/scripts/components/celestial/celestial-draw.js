import { Equator, Horizon, Illumination, RotateVector, Rotation_EQD_HOR as RotationEQDHor } from 'astronomy-engine'
...
const Camera = (observer, time, bodyName) => {
	...
	const vec = RotateVector(RotationEQDHor(time, observer), moonEQU.vec)
...
export const celestialDraw = async (context, layerWidth, layerHeight, observer, config) => {
	...
	if (altitude > 0 || variables.displayConfig.display_below_horizon) {
		...
		await getImg(`./build/assets/img/planets/${config.name.toLowerCase()}.png`)
			.then((img) => {
